                                          ...
                                          uint64_t readsize )
{
    ...
    {
        msg_Warn(s,"mp4: wanted %ld bytes, got %zd", readsize, val );
        goto error;
    }
...
static int MP4_PeekBoxHeader( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    //msg_Dbg(p_stream, "can tarck get box:%4.4s %d",(char*)&p_box->i_type, p_box->i_shortsize);
    /* Now special case */

    if( p_box->i_shortsize == 1 )
    {//box size ==1,表示后续用64位存储大小
        if( i_read < 8 )
            return 0;
               ...
               const uint32_t excludelist[], bool b_indexed )
{
      ...
      )
    {
        ...
        msg_Dbg(p_stream, " there is no box to load");
        return 0;
    }
                                         ...
                                         uint8_t *p_buffer, uint64_t i_size, uint64_t i_offset )
{
    ...
    stream_t *p_substream = vlc_stream_MemoryNew( p_stream, p_buffer, i_size,
                                                  true );
    //msg_Dbg(p_stream, "can track p_substream==NULL:%d",!p_substream?1:0);
    if( !p_substream )
        return 0;
...
{
    //msg_Dbg(p_stream, "can read avc1 box");
    ...
    //msg_Dbg(p_stream,"cantrack i_actually_read %ld,  p_peek - p_buff %ld",i_actually_read,p_peek - p_buff);

    if( i_actually_read > 78 && p_peek - p_buff > 78 )
    {
        //msg_Dbg(p_stream,"cantrack");
        MP4_ReadBoxContainerRawInBox( p_stream, p_box, p_peek, i_read,
...
static int MP4_Box_Read_Specific( stream_t *p_stream, MP4_Box_t *p_box, MP4_Box_t *p_father )
{
    ...
    for( i_index = 0; ; i_index++ )
    {
        ...
        {
            //msg_Dbg(p_stream, "can read:box %4.4s",(char*)&p_box->i_type);
            break;
        }
    }
...
MP4_Box_t *MP4_BoxGetRoot( stream_t *p_stream )
{
    ...
    if( vlc_stream_GetSize( p_stream, &i_size ) == 0 ){
        ...
        msg_Dbg(stream_t, "stream Size:%ld",i_size);
    }
