...
int SetupVideoES( demux_t *p_demux, mp4_track_t *p_track, MP4_Box_t *p_sample )
{
    ...
    else switch( p_sample->i_type )
    {
        ...
        case VLC_FOURCC( 'd', 'v', 'a', 'v' ): /* DolbyVision */
        {
            ...
            msg_Dbg(p_demux,"can track avcC==NULL:%d \n",(p_avcC==NULL)?1:0);
            if( p_avcC && BOXDATA(p_avcC) )
            {
                ...
                #else
                //can
                p_track->fmt.i_extra = sizeof(MP4_Box_data_avcC_t);
                p_track->fmt.p_extra = malloc(sizeof(MP4_Box_data_avcC_t));
                memcpy(p_track->fmt.p_extra,BOXDATA(p_avcC),sizeof(MP4_Box_data_avcC_t));
                #endif
            }
            else
            {
