...
const char vlc_module_name[] = "MP4 stream demuxer";
 ...
 *****************************************************************************/
 int   Demux   ( demux_t * );
 int   DemuxRef( demux_t *p_demux ){ (void)p_demux; return 0;}
 int   DemuxFrag( demux_t * );
...
static block_t * MP4_Block_Convert( demux_t *p_demux, const mp4_track_t *p_track, block_t *p_block )
{
    ...
    {
         printf("[%s%d]do not support!!",__FUNCTION__,__LINE__);
...
static void MP4_Block_Send( demux_t *p_demux, mp4_track_t *p_track, block_t *p_block )
{
    ...
    {
    printf("[%s%d] reorder chans !!! not support!!\n",__FUNCTION__,__LINE__);
    #if 0
...
int Open(demux_t* p_this)
{
    ...
    printf("[%s%d]%c %c %c %c \n",__FUNCTION__,__LINE__,p_peek[4], p_peek[5], p_peek[6], p_peek[7]);
    ...
    msg_Dbg(p_demux,"loadInitFrag---");

    if( LoadInitFrag( p_demux ) != VLC_SUCCESS )
        goto error;
    
    msg_Dbg(p_demux,"boxDumpStruct---");
    ...
    {
        printf( "No valid mvhd found \n" );
        goto error;
    }


    MP4_Box_t *p_rmra = MP4_BoxGet( p_sys->p_root, "/moov/rmra" );
    //rmra Reference Movie Atom,此容器不常用,屏蔽掉先
#if 0
...
static int DemuxMoov( demux_t *p_demux )
{
    ...
    msg_Dbg(p_demux, "p_sys->i_tracks :%u ",p_sys->i_tracks);
    ...
    for( i_track = 0; i_track < p_sys->i_tracks; i_track++ )
    {
        ...
        bool b = true;
        msg_Dbg(p_demux,"%d: tk->fmt:%c",i_track,"UVASD"[tk->fmt.i_cat]);
        msg_Dbg(p_demux,"tk->b_selected:%d b%d b_ok:%d, bChapters_src:%d isamp:%d, isample_count:%d",tk->b_selected,b,tk->b_ok,tk->b_chapters_source,tk->i_sample,tk->i_sample_count);
        ...
        msg_Dbg(p_demux,"tk->b_selected:%d b%d",tk->b_selected,b);
        if( tk->b_selected && !b )
        {
            MP4_TrackSelect( p_demux, tk, false );
        }
 ...
 * Control:
 *****************************************************************************/
int Control( demux_t *p_demux, int i_query, va_list args )
{
    demux_sys_t *p_sys = p_demux->p_sys;
    ...
    switch( i_query )
    {
        ...
        {
            printf("[%s%d] DEMUX_GET_PTS_DELAY ",__FUNCTION__,__LINE__);
            #if 0
            for( unsigned int i = 0; i < p_sys->i_tracks; i++ )
            {
    ...
    case AUDIO_ES:
        ...
        if( p_sys->p_meta )
        {
            ...
            if( psz_meta )
            {
                ...
                double f_gain = strtod(psz_meta,NULL);
                                   ...
                                   uint32_t *pi_sample )
{
    ...
    //msg_Dbg(p_demux,"can  Go to chunk");
        /* Go to chunk */
    ...
    *pi_chunk  = i_chunk;
    *pi_sample = i_sample;
    //msg_Dbg(p_demux,"can track chunk:%u, sample:%u",i_chunk,i_sample);
    return VLC_SUCCESS;
}
