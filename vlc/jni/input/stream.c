...
ssize_t  my_read(stream_t * s, void *buf, size_t len){
    if(g_fp == NULL){
        g_fp = fopen("native.mp4","r");
        if(g_fp == NULL){
    ...
    if(buf!=NULL){
        ...
        return fread(buf,1,len,g_fp);
    }else{
        return 0;
    }
}

int my_seek(stream_t *s, uint64_t offset){
    if(g_fp !=NULL){
...
int my_control(stream_t *s, int cmd, va_list args){
...
stream_t *vlc_stream_CommonNew(vlc_object_t *parent,
                               void (*destroy)(stream_t *))
{
//外部需要调用这个创建一个流，MP4 demuxer会从该流中读取数据，控制等等
#if 0
...
#elif 0
#if 0
//我们干脆在这里直接切断，自己实现Stream
    stream_priv_t *priv = malloc(sizeof (*priv));
    if (unlikely(priv == NULL))
        return NULL;
...
ssize_t vlc_stream_Peek(stream_t *s, const uint8_t **restrict bufp, size_t len)
{
    ...
    peekBuf = malloc(len);
    if(peekBuf == NULL){//需要外面释放，！！！
        printf("[%s%d] malloc err!\n",__FUNCTION__,__LINE__);
        return 0;
    }
    int ret =0;
    if( (ret = fread(peekBuf,1,len,g_fp)) <len){
        printf("[%s%d] read err!\n",__FUNCTION__,__LINE__);
        return 0;
    }else{
         fseek(g_fp,0-ret,SEEK_CUR);
         printf("[%s%d] read ok :%d!\n",__FUNCTION__,__LINE__,ret);
         if(bufp!=NULL){
            *bufp = peekBuf;
        }
        return ret;
...
{
#if 0
    long tellp= ftell(g_fp);
    msg_Dbg(s, "tell:%ld", tellp);
    return tellp;
#else
...
{
    msg_Dbg(s,"wang_ streamblock at:%lu",vlc_stream_Tell(s));
