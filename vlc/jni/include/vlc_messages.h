...
#define msg_Generic(o,l,fmt,...) \
    printf("%c %s:[%s::%s %d] " fmt "\n","IEWD"[l],vlc_module_name,MYFILE(__FILE__),__FUNCTION__,__LINE__,##__VA_ARGS__)

//    vlc_Log(VLC_OBJECT(o), p, vlc_module_name, __FILE__, __LINE__, \
//            __func__, __VA_ARGS__)
#define msg_Info(p_this,fmt...) \
    msg_Generic(p_this,VLC_MSG_INFO,fmt)
#define msg_Err(p_this,fmt...) \
    msg_Generic(p_this,VLC_MSG_ERR,fmt)
#define msg_Warn(p_this,fmt...) \
    msg_Generic(p_this,VLC_MSG_WARN,fmt)
#define msg_Dbg(p_this,fmt...) \
    msg_Generic(p_this,VLC_MSG_DBG,fmt)
